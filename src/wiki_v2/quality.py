...
def is_junk_chunk(text: str) -> bool:
    ...
    Фикс 2026-08-26: чистый YAML-фронтматтер страницы (page_chunk:0, спан
    «---\\ntitle: …\\n---» без тела) — служебная шапка, не контент. Раньше он
    получал вектор, выигрывал конкурс чанков по совпадению с title и после
    _strip_frontmatter превращал инжект в «шапку без чанка». Точная маска:
    спан начинается с «---», содержит «\\ntitle:» и ЗАКАНЧИВАЕТСЯ закрывающим
    «---» (спан = только фронтматтер); проза с «---»-разделителями не задевается.
    """
    if not text or not text.strip():
        return True
    t = text.strip()
    if t.startswith("---") and "\ntitle:" in t[:300]:
        ...
        if end != -1 and not t[end + 4:].strip():
            return True
    words = text.split()
    if len(words) < 6:
        return True
